...
function CarbonChart() {
  ...
  function getCarbonFootprintData(){
    CarbonFootprintService.getCarbonFootprintData(SESSION_ACCOUNT_GUID,selectedYear,selectedMonth)
      .then((response) =>{
  ...
  <Box display="flex" alignItems="center" paddingBottom={10}>
    ...
    <Select
      placeholder="Month"
      onChange={(e) => dropDownHandler1({ target: { value: parseInt(e.target.value) || 1 } })}
      width={"200px"}
      variant="filled"
      mr={2}
    >
    ...
    <Select
      placeholder="Year"
      onChange={(e) => dropDownHandler2({ target: { value: parseInt(e.target.value) || 2023 } })}
      width={"200px"}
      variant="filled"
    >
